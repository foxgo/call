...
public class CallRecordConsumer {
    ...
    public CallRecordConsumer(ObjectMapper objectMapper, MessageBatchProcessor batchProcessor) {
        this.objectMapper = objectMapper;
        this.batchProcessor = batchProcessor;
    }

    @KafkaListener(
            topics = "${call.kafka.topics.record}",
            containerFactory = "batchKafkaListenerContainerFactory"
    )
    public void consume(List<ConsumerRecord<String, String>> records, Acknowledgment acknowledgment)
            throws JsonProcessingException {
        List<CallRecordMessage> messages = records.stream()
                .map(record -> readValue(record.value(), CallRecordMessage.class))
                .toList();
        batchProcessor.processRecordBatch(messages);
