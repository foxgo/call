...
这个设计方向是对的，特别适合多实例、海量任务、号码热窗口调度场景。但当前代码实现还没有完全兑现目标设计，主要差距集中在：
...
因此，当前架构可以视为“方向正确、骨架完整、关键细节仍需补齐”的阶段。
...
| `TaskActivationService` | 把任务放入活跃调度队列 | 调度入口统一收口 |
...
| `DispatchConcurrencyLimiter` | 维护全局/租户/任务并发额度 | 当前实现未在主派发链路真正占用 |
...
设计稿里希望使用 `fairScore + weight` 做加权公平调度，但当前代码里：

- `pollNextTask()` 只是取 ZSET 最小分值任务
- 任务被取出后没有更新 fair score
- 也没有在派发后重新按“本轮实际服务量”调整得分

结果是：

- 当前实现更像“按最小分数选一个任务”
- 不是完整的加权公平轮转
- 多任务处于同一分区时，容易出现任务长期偏斜

### 11.2 活跃任务队列没有真正消费后重排

`ActiveTaskQueue.pollNextTask()` 当前只读不删，意味着：

- task 会一直停留在活跃队列头部
- 没有显式的阻塞、退队、重新入队机制
- `TaskSchedulingState` / `TaskBlockReason` 目前只做了元数据定义，未真正参与调度决策

这使得“活跃任务队列”目前更像一个简化版索引，而不是完整调度器。

### 11.3 并发限流器未在主派发链路真正占用额度

这是当前实现里最关键的风险之一。

`DispatchConcurrencyLimiter` 提供了：

- `tryAcquire()`
- `release()`
- `available()`

但 `PartitionSchedulerWorker` 当前只调用了 `available()`，没有在每次成功派发前执行 `tryAcquire()`。这会带来几个后果：

- 全局并发、租户并发限制实际上没有真正生效
- 回写和恢复阶段会执行 `release()`，计数可能变成负数
- Redis 并发计数与真实 `DIALING` 数量可能长期漂移

### 11.4 导入号码后不会自动重新激活运行中任务

当前 `CallTaskImportService` 在导入结束后：

- 会更新导入批次
- 会更新任务总量
- 但不会在任务已经 `RUNNING` 时调用 `TaskActivationService`

这意味着：

- 对于已运行任务，新增号码不一定会立即进入调度闭环
- 实际是否被及时消费，依赖其他后续事件

### 11.5 回写成功时的任务再激活偏保守但有额外噪声

当前成功回写、失败回写、retry 到期、超时恢复后都会激活任务。好处是可以尽快补位，坏处是：

- 激活可能比较频繁
- 在活跃任务队列尚未具备完整去重和阻塞语义时，会增加无效唤醒
...
1. 先修正主派发链路并发占用问题，保证 `tryAcquire()` 和 `release()` 成对闭环。
2. 给 `ActiveTaskQueue` 增加“取出后重排/阻塞/退队”能力，真正使用 `TaskSchedulingState` 和 `TaskBlockReason`。
3. 落实 fair score 更新逻辑，完成加权公平调度。
4. 在号码导入完成后，如果任务处于 `RUNNING`，立即激活任务。
5. 增加任务自动完结和调度统计收敛逻辑。

## 14. 一句话结论

当前 `call-task` 的号码调度架构已经具备可扩展的正确骨架：`MySQL 保真相、Redis 保热状态、RocketMQ 做投递、分区 lease 做多实例分工、回写/重试/恢复做事件闭环`。  
但它仍是一个“骨架已成、关键控制点未完全收口”的实现，后续最优先需要补齐公平调度和并发控制两项核心能力。
